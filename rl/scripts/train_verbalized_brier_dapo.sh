...
# Training script: Qwen3-4B-Instruct-Verbalized-Brier on DAPO-Math-17k
# Eval on DAPO-Math (val) and BeyondAIME
# tmux new -d -s brier_dapo 'bash rl/scripts/train_verbalized_brier_dapo.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_brier_dapo_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/verbalized_brier_dapo_$(date +%Y%m%d_%H%M%S).log"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-verbalized_brier_dapo}"
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT_DIR/output/checkpoints}"
OUTPUT_DIR="$OUTPUT_DIR/Qwen3-4B-Instruct-Verbalized-Brier-DAPO-$(date +%Y%m%d_%H%M%S)"
