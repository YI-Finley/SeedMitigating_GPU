#!/bin/bash
# 训练脚本: Qwen3-4B-Instruct-Confidence-Prod
# 描述: 声明级置信度 + 乘积聚合
# 对应论文: Section 3.3 - Claim-level Calibration (Product Aggregation)
...
# 输出目录
OUTPUT_DIR="/data2/SeedMitigating-output/paper_reproduction/checkpoints/confidence_prod"
rm -f "$OUTPUT_DIR/TRAINING_DONE"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "训练: Qwen3-4B-Instruct-Confidence-Prod"
echo "=========================================="
echo "策略: 声明级置信度 + 乘积聚合"
echo "奖励: R = 2(∏p_i)·valid(y) - (∏p_i)²"
echo "聚合方式: Product"
echo "输出: $OUTPUT_DIR"
...
python3 -m verl.trainer.main_ppo \
    ...
    custom_reward_function.name=compute_score_claim_product \
    trainer.logger='["console","wandb"]' \
    trainer.project_name=behavioral_calibration_paper \
    trainer.experiment_name=confidence_prod \
