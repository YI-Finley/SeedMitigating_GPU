...
# Training script: Qwen3-4B-Instruct-Verbalized-Brier-RLOO (resume)
# Description: Resume Verbalized Confidence + Brier Score reward training with RLOO
# Usage examples:
#   bash rl/scripts/train_verbalized_brier_rloo_resume.sh
#   RESUME_ROOT=/path/to/run bash rl/scripts/train_verbalized_brier_rloo_resume.sh
#   RESUME_DIR=/path/to/run/global_step_240 bash rl/scripts/train_verbalized_brier_rloo_resume.sh
# tmux new -d -s brier_rloo_resume 'bash rl/scripts/train_verbalized_brier_rloo_resume.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/resume_verbalized_brier_rloo_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/resume_verbalized_brier_rloo_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-verbalized_brier_triviaqa_rloo}"

RESUME_ROOT="${RESUME_ROOT:-/root/code/SeedMitigating/output/checkpoints/Qwen3-4B-Instruct-Verbalized-Brier-RLOO-20260313_154448}"
if [ -z "${RESUME_DIR:-}" ]; then
    RESUME_DIR="$(find "$RESUME_ROOT" -maxdepth 1 -type d -name 'global_step_*' | sort -V | tail -n 1)"
fi
if [ -z "$RESUME_DIR" ] || [ ! -d "$RESUME_DIR" ]; then
    echo "[ERROR] Resume checkpoint not found. RESUME_ROOT=$RESUME_ROOT RESUME_DIR=${RESUME_DIR:-<empty>}"
    exit 1
fi
...
echo "=========================================="
echo "Training: Qwen3-4B-Instruct-Verbalized-Brier"
echo "=========================================="
echo "Strategy: Verbalized Confidence + Brier Score"
echo "Reward: Eq.3 (Paper Section 3.2.2)"
echo "Algorithm: RLOO"
...
echo "Training complete: Verbalized Brier"
