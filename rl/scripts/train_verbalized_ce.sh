...
# Usage examples:
#   bash rl/scripts/train_verbalized_ce.sh
#   SAMPLE_SIZE=100 bash rl/scripts/train_verbalized_ce.sh
#   USE_SAMPLE=0 TRAIN_FILES=/path/train.parquet VAL_FILES=/path/val.parquet bash rl/scripts/train_verbalized_ce.sh
# tmux new -d -s ce 'bash rl/scripts/train_verbalized_ce.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_ce_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
# tmux list-sessions
# tmux attach-session -t ce
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/verbalized_ce_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
OUTPUT_DIR="${OUTPUT_DIR}/Qwen3-4B-Instruct-Verbalized-CE-$(date +%Y%m%d_%H%M%S)"
...
echo "HCCL port range: $HCCL_PORT_RANGE"
echo "Algorithm: GRPO"
