...
# Training script: Qwen3-4B-Instruct-Verbalized-Brier (resume)
# Description: Verbalized Confidence + Brier reward (Section 3.2.2)
# Usage examples:
#   bash rl/scripts/train_verbalized_brier_resume.sh
#   SAMPLE_SIZE=100 bash rl/scripts/train_verbalized_brier_resume.sh
#   USE_SAMPLE=0 TRAIN_FILES=/path/train.parquet VAL_FILES=/path/val.parquet bash rl/scripts/train_verbalized_brier_resume.sh
# tmux new -d -s brier_resume 'bash rl/scripts/train_verbalized_brier_resume.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_brier_resume_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/resume_verbalized_brier_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-verbalized_brier_triviaqa}"

RESUME_ROOT="/root/code/SeedMitigating/output/checkpoints/Qwen3-4B-Instruct-Verbalized-Brier-20260307_235307"
...
echo "=========================================="
echo "Strategy: Verbalized Confidence + Brier"
echo "Reward: Brier (Paper Section 3.2.2)"
