...
# Training script: Qwen3-4B-Instruct-Verbalized-CE (resume, 4 NPU)
# Description: Verbalized Confidence + Cross-Entropy reward (Section 3.2.2)
# Usage examples:
#   bash rl/scripts/train_verbalized_ce_resume_4npu.sh
#   SAMPLE_SIZE=100 bash rl/scripts/train_verbalized_ce_resume_4npu.sh
#   USE_SAMPLE=0 TRAIN_FILES=/path/train.parquet VAL_FILES=/path/val.parquet bash rl/scripts/train_verbalized_ce_resume_4npu.sh
#   RESUME_ROOT=/root/code/SeedMitigating/output/checkpoints/Qwen3-4B-Instruct-Verbalized-CE-20260307_235307 bash rl/scripts/train_verbalized_ce_resume_4npu.sh
# tmux new -d -s ce_resume_4npu 'bash rl/scripts/train_verbalized_ce_resume_4npu.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_ce_resume_4npu_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
export PYTHONPATH="$ROOT_DIR/verl:$ROOT_DIR/rl/compat:${PYTHONPATH:-}"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/verbalized_ce_resume_4npu_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-verbalized_ce_triviaqa_4npu}"
MODEL_PATH="${MODEL_PATH:-Qwen/Qwen3-4B-Instruct-2507}"

RESUME_ROOT="${RESUME_ROOT:-/root/code/SeedMitigating/output/checkpoints/Qwen3-4B-Instruct-Verbalized-CE-20260307_235307}"

if [ -n "${RESUME_DIR:-}" ]; then
    echo "[INFO] Using user-specified RESUME_DIR: $RESUME_DIR"
elif [ -f "$RESUME_ROOT/latest_checkpointed_iteration.txt" ]; then
    LAST_ITER=$(tr -d '[:space:]' < "$RESUME_ROOT/latest_checkpointed_iteration.txt")
    RESUME_DIR="$RESUME_ROOT/global_step_${LAST_ITER}"
    echo "[INFO] Auto-detected RESUME_DIR from latest_checkpointed_iteration.txt: $RESUME_DIR"
else
    RESUME_DIR=$(find "$RESUME_ROOT" -maxdepth 1 -type d -name 'global_step_*' | sort -V | tail -n 1)
    echo "[INFO] Auto-detected latest RESUME_DIR by directory scan: $RESUME_DIR"
fi

if [ ! -d "$RESUME_ROOT" ]; then
    echo "[ERROR] RESUME_ROOT does not exist: $RESUME_ROOT"
    exit 1
fi

if [ -z "$RESUME_DIR" ] || [ ! -d "$RESUME_DIR" ]; then
    echo "[ERROR] RESUME_DIR is invalid: $RESUME_DIR"
    echo "[INFO] Available checkpoints under $RESUME_ROOT:"
    find "$RESUME_ROOT" -maxdepth 1 -type d -name 'global_step_*' | sort -V
    exit 1
fi

SOURCE_FSDP_CONFIG="$RESUME_DIR/actor/fsdp_config.json"
SOURCE_WORLD_SIZE=""
if [ -f "$SOURCE_FSDP_CONFIG" ]; then
    SOURCE_WORLD_SIZE=$(grep -o '"world_size"[[:space:]]*:[[:space:]]*[0-9]\+' "$SOURCE_FSDP_CONFIG" | grep -o '[0-9]\+' | head -n 1)
fi

ACTOR_LOAD_CONTENTS_OVERRIDE=""
CONVERTED_RESUME_ROOT="${CONVERTED_RESUME_ROOT:-${RESUME_ROOT}_ws${NPU_COUNT}_modelresume}"
FORCE_RESHARD_CHECKPOINT="${FORCE_RESHARD_CHECKPOINT:-1}"
RESHARD_MASTER_PORT="${RESHARD_MASTER_PORT:-40123}"

if [ -n "$SOURCE_WORLD_SIZE" ] && [ "$SOURCE_WORLD_SIZE" != "$NPU_COUNT" ]; then
    SOURCE_STEP_DIR_NAME="$(basename "$RESUME_DIR")"
    CONVERTED_RESUME_DIR="$CONVERTED_RESUME_ROOT/$SOURCE_STEP_DIR_NAME"
    CONVERSION_META_FILE="$CONVERTED_RESUME_DIR/conversion_meta.json"
    echo "[INFO] Detected cross-world-size resume: source=$SOURCE_WORLD_SIZE target=$NPU_COUNT"
    echo "[INFO] Converting checkpoint to model-only $NPU_COUNT-card FSDP shards: $CONVERTED_RESUME_DIR"

    NEED_RESHARD=0
    if [ "$FORCE_RESHARD_CHECKPOINT" = "1" ] || [ ! -d "$CONVERTED_RESUME_DIR/actor" ]; then
        NEED_RESHARD=1
    elif [ ! -f "$CONVERSION_META_FILE" ]; then
        echo "[INFO] Converted checkpoint metadata missing, forcing re-conversion for compatibility."
        NEED_RESHARD=1
    elif ! grep -q '"target_mesh_dim_names"[[:space:]]*:[[:space:]]*\[[[:space:]]*"fsdp"[[:space:]]*\]' "$CONVERSION_META_FILE"; then
        echo "[INFO] Converted checkpoint metadata is legacy/incompatible, forcing re-conversion."
        NEED_RESHARD=1
    fi

    if [ "$NEED_RESHARD" = "1" ]; then
        mkdir -p "$CONVERTED_RESUME_ROOT"
        if [ -n "$VERL_NPU_DEVICE_MAP" ]; then
            export ASCEND_RT_VISIBLE_DEVICES="$VERL_NPU_DEVICE_MAP"
        fi
        torchrun --nproc_per_node="$NPU_COUNT" --master_port="$RESHARD_MASTER_PORT" \
            "$ROOT_DIR/rl/scripts/convert_fsdp_checkpoint_world_size.py" \
            --source-checkpoint-dir "$RESUME_DIR" \
            --output-checkpoint-dir "$CONVERTED_RESUME_DIR" \
            --copy-data-state
        if [ -n "$VERL_NPU_DEVICE_MAP" ]; then
            unset ASCEND_RT_VISIBLE_DEVICES
        fi
        printf '%s\n' "${SOURCE_STEP_DIR_NAME#global_step_}" > "$CONVERTED_RESUME_ROOT/latest_checkpointed_iteration.txt"
    else
        echo "[INFO] Reusing existing converted checkpoint: $CONVERTED_RESUME_DIR"
    fi

    RESUME_DIR="$CONVERTED_RESUME_DIR"
    OUTPUT_DIR="${OUTPUT_DIR:-$CONVERTED_RESUME_ROOT}"
    ACTOR_LOAD_CONTENTS_OVERRIDE="actor_rollout_ref.actor.checkpoint.load_contents=['model']"
    if [ -d "$RESUME_DIR/actor/huggingface" ]; then
        MODEL_PATH="$RESUME_DIR/actor/huggingface"
    fi
else
    OUTPUT_DIR="${OUTPUT_DIR:-$RESUME_ROOT}"
fi
...
echo "=========================================="
echo "Training: Qwen3-4B-Instruct-Verbalized-CE (4 NPU)"
echo "=========================================="
...
echo "Model init path: $MODEL_PATH"
if [ -n "$ACTOR_LOAD_CONTENTS_OVERRIDE" ]; then
    echo "Resume load mode: actor model-only (cross-world-size conversion)"
fi
echo ""
...
EXTRA_HYDRA_ARGS=()
if [ -n "$ACTOR_LOAD_CONTENTS_OVERRIDE" ]; then
    EXTRA_HYDRA_ARGS+=("$ACTOR_LOAD_CONTENTS_OVERRIDE")
fi
...
echo "Training complete: Verbalized CE (4 NPU)"
