...
# tmux new -d -s brier 'bash rl/scripts/train_verbalized_brier.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_brier_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/verbalized_brier_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
OUTPUT_DIR="${OUTPUT_DIR}/Qwen3-4B-Instruct-Verbalized-Brier-$(date +%Y%m%d_%H%M%S)"
