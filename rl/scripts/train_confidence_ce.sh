#!/bin/bash
# 训练脚本: Qwen3-4B-Instruct-Confidence-CE
...
# 输出目录
OUTPUT_DIR="/data2/SeedMitigating-output/paper_reproduction/checkpoints/confidence_ce"
rm -f "$OUTPUT_DIR/TRAINING_DONE"
mkdir -p "$OUTPUT_DIR"

echo "=========================================="
echo "训练: Qwen3-4B-Instruct-Confidence-CE"
echo "=========================================="
echo "策略: Verbalized Confidence + Cross-Entropy"
echo "奖励: R = [log((1-ε)/ε)]^(-1) * [valid(y)·log(p'/ε) + ...]"
echo "风险阈值分布: Beta分布"
echo "算法: GRPO (提升训练稳定性)"
echo "输出: $OUTPUT_DIR"
...
python3 -m verl.trainer.main_ppo \
    ...
    +custom_reward_function.reward_kwargs.epsilon=0.01 \
    trainer.logger='["console","wandb"]' \
    trainer.project_name=behavioral_calibration_paper \
    trainer.experiment_name=confidence_ce \
