...
# Training script: Qwen3-4B-Instruct-Verbalized-Brier
# Description: Verbalized Confidence + Brier Score reward (Section 3.2.2)
# Usage examples:
...
# tmux new -d -s brier_rloo 'bash rl/scripts/train_verbalized_brier_rloo.sh; code=$?; echo "[EXIT_CODE]=$code"; echo "[LAST_LOG]"; ls -t output/logs/verbalized_brier_rloo_*.log 2>/dev/null | head -1 | xargs -r -I{} tail -n 120 {}; exec bash'
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/verbalized_brier_rloo_sample${SAMPLE_SIZE_TRAIN}_${SAMPLE_SIZE_VAL}_$(date +%Y%m%d_%H%M%S).log"
...
OUTPUT_DIR="${OUTPUT_DIR:-/root/code/SeedMitigating/output/checkpoints/Qwen3-4B-Instruct-Verbalized-Brier-RLOO-$(date +%Y%m%d_%H%M%S)}"
