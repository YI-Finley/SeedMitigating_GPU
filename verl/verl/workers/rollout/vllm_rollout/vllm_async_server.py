...
class vLLMHttpServerBase:
    ...
    async def generate(
        ...
        # Ensure prompt token ids are plain Python ints for vLLM v1 detokenizer.
        def _normalize_prompt_ids(value):
            if value is None:
                return []
            if isinstance(value, (list, tuple)):
                out = []
                for item in value:
                    out.extend(_normalize_prompt_ids(item))
                return out
            if hasattr(value, "tolist"):
                try:
                    return _normalize_prompt_ids(value.tolist())
                except Exception:
                    pass
            try:
                return [int(value)]
        ...
        if len(prompt_ids) == 0:
            fallback_id = None
            try:
                fallback_id = self.model_config.tokenizer.eos_token_id
            except Exception:
                fallback_id = None
            if fallback_id is None:
                try:
                    fallback_id = self.model_config.hf_config.eos_token_id
                except Exception:
                    fallback_id = None
            if fallback_id is None:
                fallback_id = 0
