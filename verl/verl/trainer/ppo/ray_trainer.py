...
class RayPPOTrainer:
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            sample_gts = [
                (item.non_tensor_batch.get("reward_model") or {}).get("ground_truth", None) for item in batch
            ]
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            ground_truths = [
                (item.non_tensor_batch.get("reward_model") or {}).get("ground_truth", None) for item in test_batch
            ]
            ...
            # 去除里面的metrics
            if "metrics" in result["reward_extra_info"].keys():
                for k, v in result["reward_extra_info"]['metrics'].items():
                    tmp_metrics[k].append(v)
                result["reward_extra_info"].pop("metrics")
        ...
        if len(tmp_metrics) > 0:
            for k, vals in tmp_metrics.items():
                if len(vals) == 0:
                    continue

                # make sure each v is a python float
                clean_vals = []
                for v in vals:
                    if hasattr(v, "item"):  # torch scalar / numpy scalar
                        clean_vals.append(float(v.item()))
                    else:
                        clean_vals.append(float(v))

                metric_dict[f"val-aux/custom/{k}"] = float(np.mean(clean_vals))
