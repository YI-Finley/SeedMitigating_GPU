...
class Worker(WorkerHelper):
    ...
    def _setup_env_cuda_visible_devices(self):
        ...
        if is_ray_noset_visible_devices:
            ...
            os.environ["LOCAL_RANK"] = str(local_rank_int)
            
            # 支持 NPU 设备映射：当设置了 VERL_NPU_DEVICE_MAP 时，将 local_rank 映射到实际物理设备
            if is_npu_available and "VERL_NPU_DEVICE_MAP" in os.environ:
                device_map = [int(x.strip()) for x in os.environ["VERL_NPU_DEVICE_MAP"].split(",")]
                if local_rank_int < len(device_map):
                    physical_device = device_map[local_rank_int]
                    print(f"[VERL] Mapping local_rank {local_rank_int} to physical NPU device {physical_device}")
                    get_torch_device().set_device(physical_device)
                else:
                    get_torch_device().set_device(local_rank_int)
            else:
                get_torch_device().set_device(local_rank_int)
